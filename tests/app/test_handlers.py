...
def test_parse(client: TestClient, read_from_test_data: ReadFromTestDataFunc, clothing_with_color: Clothing) -> None:
...
def test_upload_image_via_file(client: TestClient, read_from_test_data: ReadFromTestDataFunc) -> None:
    ...
    assert resp.json() == {"url": "https://storage.yandexcloud.net/w2w/images/kartinka.gif"}


def test_upload_image_via_link(client: TestClient, read_from_test_data: ReadFromTestDataFunc) -> None:
    app.dependency_overrides[GetBinary] = lambda: AsyncMock(
    ...
    resp = client.post("/upload_image_via_link", json={"url": "https://avatarko.ru/img/kartinka/1/avatarko_anonim.jpg"})
